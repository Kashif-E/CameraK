...
### Fixed
- **iOS auto-save wrote a second copy of every capture to the camera roll** (#160, follow-up): with `Directory.PICTURES`/`DCIM`, `takePictureToFile()` saves to the Photos library and the iOS `ImageSaverPlugin` saver then saved the same image there again, since it always targets the Photos library whatever `directory` is configured. Auto-save now skips captures the controller has already written to its own destination. Android and desktop are unchanged: their savers write into their own subfolder (`customFolderName`, or `CameraK` on Android), so that copy is a distinct file rather than a duplicate.
