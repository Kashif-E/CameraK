...
### Added
- **`CameraController.getDirectory()`**: reports the [Directory] captures are written to. Lets a plugin tell whether `takePictureToFile()` has already stored the image where the plugin would save it.

### Fixed
- **Auto-save wrote a second copy of every capture to the gallery** (#160, follow-up): `takePictureToFile()` already stores the capture where the plugin then saved it again, so an auto-saving `ImageSaverPlugin` produced two identical images per shutter press. On iOS the saver always targets the Photos library, which is where `PICTURES`/`DCIM` captures already are; on Android the capture lands in `<directory>/CameraK` and the saver's default folder is that same `CameraK`. Auto-save now skips captures the controller has already written to the saver's own destination, and keeps saving the rest (a configured `customFolderName` on Android is a different folder, so that copy is kept; on desktop the capture goes to a temp file, so the saver's copy is the only one). Note on iOS: for skipped captures the plugin's `onImageSaved`/`onImageSavedFailed` callbacks no longer fire, since nothing is saved; use the path returned by `takePictureToFile()`.
- **iOS capture errors said nothing** (#160, follow-up): `CameraException.ConfigurationError`/`CaptureError` accepted a message and dropped it instead of passing it to `Exception`, so both the log line and the `ImageCaptureResult.Error` handed to the caller carried only a class name.
- **iOS capture bookkeeping could race** (#160, follow-up): the one-shot guard deciding which of the photo callback, the error callback or cancellation completes a capture was a plain flag read from several threads, so two could pass it, decrementing the burst counter twice and clearing `isCapturing` while a file write was still running. It is now atomic.
- **iOS image capture listeners stopped firing after the first `takePictureToFile()`** (#160): `takePictureToFile()` overwrote the `onPhotoCapture` callback that feeds `addImageCaptureListener` and never restored it, so from the first file capture onward every registered listener was silently a no-op for the life of the controller (the list was untouched and no error was raised, hence the silence). `setupCamera()` is now the only writer of the photo/error callbacks: a capture parks its handler in a slot that the callback drains, so listeners fire on every capture — including file captures, matching Android — and the `ByteArray` conversion still only runs when a listener is registered. Errors are also logged again instead of being swallowed by a completed capture's handler, listeners now receive an exact-length array they own (previously a pooled buffer that could be longer than the photo and was recycled while they were still reading it), and a cancelled capture no longer decrements the burst counter twice.
- **`kotlinx-coroutines-test` shipped to consumers, breaking `Dispatchers.Main`** (#165): `cameraK` declared `kotlinx-coroutines-test` as an `api` dependency of `commonMain`, so it landed in the published POM at runtime scope and reached every app that depends on `camerak` — or on any plugin, since each one re-exports the core with `api`. That artifact registers a `TestMainDispatcherFactory` through `ServiceLoader`, which takes over `Dispatchers.Main`; an app that never calls `Dispatchers.setMain()` then dies with *"Dispatchers.Main was accessed when the platform dispatcher was absent and the test dispatcher was unset"*. Reported on Desktop/Windows, but the leak was on every target. The dependency is now test-only. Consumers who were unknowingly relying on it for their own tests must declare it themselves; the `configurations.all { exclude(...) }` workaround is no longer needed.
